...
class Ngo
{
    ...
    static function getFeaturedRecruiters($limit = 4)
    {
        ...
        self::$ci->db->join('jobs as j', 'u.id = j.user_id', 'LEFT');
        self::$ci->db->where('u.role_id', '4');
        self::$ci->db->where('u.is_featured', '1');
        self::$ci->db->group_by('j.user_id');
        self::$ci->db->order_by('j.hit_count', 'desc');
        self::$ci->db->limit($limit);
        return self::$ci->db->get()->result();
    }
    ...
    static function getRecentJobs($limit = 6)
    {
        ...
        self::$ci->db->join('job_categories as c', 'j.job_category_id = c.id', 'LEFT');
        self::$ci->db->join('users as u', 'u.id = j.user_id', 'LEFT');
        self::$ci->db->where('j.status', 'Publish');
        self::$ci->db->where('j.created_at BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE()');
        self::$ci->db->limit($limit);
        return self::$ci->db->get()->result();
    }
    ...
    static function getAllJobsByCategories()
    {
        ...
        self::$ci->db->select("({$job_qty}) as posts");
        self::$ci->db->from('job_categories as c');
        self::$ci->db->order_by('c.name','ASC');
        $categories = self::$ci->db->get()->result();

        $html = '<ul>';
        foreach ($categories as $cat) {
            if($cat->posts){
                $html .= "<li><a href=\"job-search?cat={$cat->id}\">";
...
//        self::$ci->db->where('status', 'Publish');
    ...
    static function getAllJobsByOrganizationTypes()
    {
        ...
        self::$ci->db->where('status', 'Publish');
        self::$ci->db->where('deadline >=', date('Y-m-d'));
        self::$ci->db->where('u.org_type_id', 'o.id', false);
        $job_qty = self::$ci->db->get_compiled_select('users as u');

        //Get All Organization Types
        self::$ci->db->select('o.id as id, o.name as name');
        self::$ci->db->select("({$job_qty}) as posts");
//        self::$ci->db->where('u.role_id', 4);
        self::$ci->db->from('users as u');
        self::$ci->db->join('organization_types as o', 'o.id = u.org_type_id', 'LEFT');
        self::$ci->db->limit(30);
        $org_types = self::$ci->db->get()->result();

        $html = '<ul>';
        foreach ($org_types as $type) {
            if($type->posts){
                $url = site_url("job-search?org={$type->id}");
                $html .= "<li><a href=\"{$url}\">";
    ...
    static function jobAndCompanyCount()
    {
        self::$ci->db->from('jobs');
        self::$ci->db->where('status', 'Published');
        $total_job = self::$ci->db->count_all_results();

        self::$ci->db->from('users');
        self::$ci->db->where('role_id', 4);
        self::$ci->db->where('status', 'Active');
        $total_company = self::$ci->db->count_all_results();

        return [
            'total_job' => number_format($total_job),
            'total_company' => number_format($total_company)
        ];
    }
