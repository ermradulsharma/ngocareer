...
class Frontend_model extends CI_Model {
    ...
    private function __job_search($cat_id,$org,$type_id,$country_id,$keyword,$lat,$lng,$salary_range,$posted,$deadline,$sort_by,$location = null) {
        ...
        if ($cat_id)    { $this->db->where('job_category_id', $cat_id); }
        if ($org)       { $this->db->where('u.org_type_id', $org); }
        if ($type_id)   { $this->db->where('job_type_id', $type_id); }
        if ($country_id)   { $this->db->where('j.country_id', $country_id); }
        if ($posted)    { $this->db->where('j.created_at >=', $posted); }
        if ($deadline)  { $this->db->where('deadline <=', $deadline); }
        if ($type_id)   { $this->db->where('job_type_id', $type_id); }
    ...
    private function __company_search($q,$org_type_id) {
        if($org_type_id) { $this->db->where('org_type_id', $org_type_id); }
        if ($q) {
            $this->db->group_start();
    ...
    private function __event_search($cat_id, $keyword, $lat, $lng) {
        ...
        if ($cat_id)    { $this->db->where('e.event_category_id', $cat_id); }
