...
class Sector extends Admin_controller
{
    ...
    public function index()
    {
        ...
        $config['base_url'] = build_pagination_url(Backend_URL . 'job/sector/', 'start');
        $config['first_url'] = build_pagination_url(Backend_URL . 'job/sector/', 'start');

        $config['per_page'] = 25;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Sector_model->total_rows($q);
        $categorys = $this->Sector_model->get_limit_data($config['per_page'], $start, $q);
        ...
        print_r($data);
        die;
        $this->viewAdminContent('job/Sector/index', $data);
    }
