...
class Mail extends MX_Controller {
    ...
    public function cronReportToDev($msg) {
        $this->send_to  = 'imrancse019@gmail.com';
        $this->subject  = "{$this->site_title} || Job Alert Report";        
        $this->body     = $msg;
        $this->sendJobAlertMail($msg);
    }

    //This function use for job alert mail send with cron job
    public function sendJobAlerts($option = []) {
        $this->send_to = $option['send_to'];
        //$this->from_name = $option['from_name'];
        $this->subject = $option['subject'];
        $this->body = $option['body'];
        $url = $option['unsubscribe_url'];
        $this->message = '<a href="'.$url.'">Unsubscribe</a>';
        $this->log();
        $this->save_in_db('jobAlert', $option['candidate_id'], 1);
        return $this->sendJobAlertMail();
    }
...
//        $config['charset'] = 'iso-8859-1';
        $config = array(
            ...
            "smtp_port" => 465,
//            "smtp_user" => "i2stsdev@gmail.com",
//            "smtp_pass" => "i2sts123!@#",
            "smtp_user" => "ngocareeruk@gmail.com",
            "smtp_pass" => "G36action39",
            "mailtype"  => "html", 
            "charset"   => "utf-8",
            "newline" => "\r\n",
            "smtp_timeout" => '60',
...
//        $this->email->from('customer.support@ngocareer.com', 'NGO Career');
        $this->email->from('contact@ngocareer.com', 'NGO Career');
        $this->email->to($this->send_to);
        $this->email->reply_to('contact@ngocareer.com', 'NGO Career');
...
//        $to = "harishmanoharan90@gmail.com";
//        $subject = "My subject";
//        $txt = "Hello world!";
//        $headers = "From: i2stsdev@gmail.com" . "\r\n";
//        mail($to,$subject,$txt,$headers);
        ...
        if ( $this->email->send(FALSE) ) {
            ...
            return ajaxRespond('Failed', '<p class="ajax_error">Mail Not Sent! Please try Again</p>'.$this->email->print_debugger());
        }
    }

    private function sendJobAlertMail($link = '') {
        $config = array(
            "protocol" => "sendmail",
            "starttls"  => true,
            "smtp_host" => "smtp.gmail.com",
        ...
        $this->email->from("customer.support@ngocareer.com", "NGO Career");
        $this->email->to($this->send_to);
        $this->email->reply_to('customer.support@ngocareer.com', 'NGO Career');
