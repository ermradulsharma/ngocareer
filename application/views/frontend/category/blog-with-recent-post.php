<div class="container blog">
    <div class="row">
    <h1>NGO Career Advice</h1>
    <div class="col-sm-9 no-padding">

        <?php if ($posts_data) { ?>
            <img src="assets/theme/images/blog-banner.jpg" class="img-responsive">
            <?php foreach ($posts_data as $post) : ?>
                <div class="news-list">
                    <div class="row">
                        <div class="col-sm-3">
                            <?php echo getPhotoDefault($post->thumb, 'img-responsive', 190, 190); ?>
                            <!--<img src="<?php //echo getPhoto($post->thumb); ?>" class="img-responsive" alt="">-->
                        </div>
                        <div class="col-sm-9">
                            <h3><?php echo $post->post_title; ?></h3>

                            <p><?php echo getShortContent($post->content, 250); ?></p>
                            <p><a class="global-button" href="<?php echo $post->post_url; ?>">Read More</a></p>
                        </div>
                    </div>                
...
<!--                <input type="search" class="search-field form-control" placeholder="Search ..." value="--><?php //echo $q; ?><!--" name="q" title="Search for:">-->
...
<!--            </div>-->
            <div class="box-right-blog">
                ...
                <div style="padding-bottom: 15px;">
                    <form action="" method="post" id="archive_form">
                        <select class="form-control" id="archive_year">
                            <?php echo numeric_dropdown(2019, date('Y'), 1, date('Y')); ?>
                        </select>
                    </form>
                </div>
                <div id="archive_ul"></div>
            </div>
...
<script type="text/javascript">
    function archive_year(){
        ...
        $.ajax({
            url: 'frontend/blog_archive_ul',
            type: "POST",
            dataType: "json",
            cache: false,
            data: { year: year },
            beforeSend: function(){
                $('#archive_ul').html('<p class="ajax_processing">Please Wait... Loading....</p>');
            },
            success: function( jsonData ){
                $('#archive_ul').html(jsonData.Msg);
            }
        });
    }
