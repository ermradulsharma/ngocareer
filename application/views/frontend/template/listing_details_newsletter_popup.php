...
<button type="button" class="btn btn-link" data-toggle="modal" data-target="#ldnModal">
    <strong style="color: #F74E05;">Alert me to jobs like these: <?php echo $job_title; ?></strong>
</button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form name="newsletter" id="ldp_newsletter" action="">
                    ...
                    <div id="captcha_pop_msg"></div>
                    <div id="ldp_pop_msg"></div>
                </form>
            </div>
...
<script type="text/javascript">
    function LDSubscribePopUp() {
        ...
        if (validateEmail(email) === false || !email) {
            $('#ldp_pop_msg').html('<span class="text-danger">Invalid Email</span>').show();
            error = 1;
        } else {
            $('#ldp_pop_msg').hide();
        }
        ...
        if (validateEmail(confirm_email) === false || !confirm_email) {
            $('#ldp_pop_msg').html('<span class="text-danger">Invalid Confirm Email</span>').show();
            error = 1;
        } else {
            $('#ldp_pop_msg').hide();
        }
        if(email != confirm_email) {
            $('#ldp_pop_msg').html('<span class="text-danger">Email and Confirm email must match</span>').show();
            error = 1;
        }

        var v = grecaptcha.getResponse();
        if(v.length == 0) {
            document.getElementById('captcha_pop_msg').innerHTML="<span style='color: red'>You can't leave Captcha Code empty</span>";
            error = 1;
        } else {
            document.getElementById('captcha_pop_msg').innerHTML="";
        }
        ...
        if (error === 0) {
            $.ajax({
                ...
                success: function (jsonData) {
                    $('#ldp_pop_msg').html(jsonData.Msg).slideDown('slow');
                    if (jsonData.Status === 'OK') {
                        $('#ldp_pop_msg').delay(5000).slideUp('slow');
